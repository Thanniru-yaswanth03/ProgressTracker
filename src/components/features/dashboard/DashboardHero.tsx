...
export function DashboardHero({ userName, data }: DashboardHeroProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl p-6 sm:p-8 border border-slate-800/80 glass-panel shadow-2xl">
      {/* Background ambient glow */}
      <div className="absolute top-0 right-0 -mr-20 -mt-20 w-80 h-80 rounded-full bg-gradient-to-br from-indigo-500/20 via-sky-500/10 to-transparent blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6">
        ...
        <div className="space-y-2 flex-1">
          ...
          <h1 className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight">
            Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-sky-300 to-amber-300">{userName}</span>
          </h1>
        ...
        <div className="flex flex-row lg:flex-col items-center justify-center p-4 sm:p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shrink-0 gap-4 min-w-[200px]">
          ...
          <div className="text-left lg:text-center">
            <div className="text-xs font-bold text-slate-200">
              {rate === 100 ? "All Goals Smashed! 🎉" : rate >= 50 ? "Great Momentum 🔥" : "Keep Building ⚡"}
            </div>
            <div className="text-[11px] text-slate-400 mt-0.5">
              {data.todayTasksCompleted + data.todayHabitsCompleted} of {data.todayTasksTotal + data.todayHabitsTotal} daily items done
            </div>
          </div>
        </div>
      </div>

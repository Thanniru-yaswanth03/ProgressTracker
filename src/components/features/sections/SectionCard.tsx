...
export function SectionCard({ section, onEdit, onDelete }: SectionCardProps) {
  ...
  return (
    <Card className="group flex flex-col justify-between border-slate-800/80 hover:border-slate-700/90 transition-all duration-200 hover:shadow-xl hover:shadow-indigo-500/5 p-5 relative overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-base text-slate-100 group-hover:text-white truncate transition-colors">
                {section.name}
              </h3>
              <p className="text-[10px] text-slate-500">Created {formattedDate}</p>
            </div>
          </div>

          ...
          <div className="h-44 flex items-end justify-between gap-2 pt-6 pb-2">
            {weeklyOverview.days.map((day) => {
              ...
              return (
                ...
                >
                  {/* Tooltip on hover */}
                  <div className="absolute -top-8 px-2 py-1 rounded-md bg-slate-950 border border-slate-700 text-[10px] font-bold text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-lg">
                    {day.dayLabel} ({day.date}): {label}
                  </div>

                  {/* Value label above bar */}
                  <span className="text-[10px] font-bold text-slate-400 mb-1 opacity-80 group-hover:text-white transition-colors">
                    {val > 0 ? label : "0"}
                  </span>

                  {/* Bar */}
                  <div className="w-full max-w-[36px] bg-slate-950/60 rounded-xl overflow-hidden p-0.5 border border-slate-800/80 flex items-end">
                    <div
            ...
            {monthlyOverview.weekTrends.map((wt) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full max-w-[56px] bg-slate-950/60 rounded-xl overflow-hidden p-0.5 border border-slate-800/80 flex items-end">
                    <div
                      className={`w-full rounded-lg transition-all duration-500 shadow-sm ${barColor}`}
                      style={{ height: `${heightPercent}%` }}
                    />
                  ...
                  <span className="text-[11px] font-bold mt-2 text-slate-400">
                    {wt.weekLabel}
                  </span>
                </div>
              );
